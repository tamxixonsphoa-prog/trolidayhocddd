...
export default function App() {
  ...
  const callGeminiWithFallback = async (parts: any[]) => {
    ...
    const getErrCode = (err: any): number => {
      try {
        // err.message may be raw JSON string
    ...
    const getRetryDelay = (err: any): number => {
      try {
        ...
        if (retryInfo?.retryDelay) {
          return parseInt(retryInfo.retryDelay) || 60;
        }
      } catch { /* ignore */ }
      return 60;
    };
    ...
    if (code === 429) {
      ...
      throw new Error(`⚠️ Đã vượt quá giới hạn API miễn phí. Vui lòng chờ khoảng ${delay} giây rồi thử lại, hoặc kiểm tra hạn mức tại https://ai.dev/rate-limit`);
    } else if (code === 404) {
